...
int main() {
    ...
    // Variables for each denomination count.
    ...
    // Calculate counts for each denomination using pass-by-reference to update remaining change.
    ...
    // Display only denominations that are used.
...
// Function to calculate change or return error code -1.
int calculate_change_amount(int cost, int paid) {
    if (paid >= cost) {
        return paid - cost;
    } else {
...
// Function to calculate count and update the remaining change via pointer.
int count_denomination(int *change_remaining, int denomination) {
    int count = *change_remaining / denomination;
    *change_remaining = *change_remaining % denomination; 
    // Update the value at the pointer address.
    return count;
}